            ...
            for j = 1:2:length(filterStrings{i}) %for combined filters
                if (strfind(lower(filterStrings{i}{j}),'<function>')) %this is a function call
                    ...
                    if isempty(tmpresult)
                        error('Cannot find data. Make sure that chinfo is created and target channels are defined.')
                    end
                    tmpresultsorted = [];
                    ...
                    f(an).data = setfield(f(an).data,filterStrings{i}{j},{g},{tmpresultsorted});   %fixed 7/15/13 MK AG
