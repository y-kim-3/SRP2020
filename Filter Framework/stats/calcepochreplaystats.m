...
for eventindex = 1:length(decodefilter(animalnum).output{1}(epochnum).eventdata)
    ...
    if ((endevent-startevent) < 2)
        ...
        if (length(activespiketimes) >= 5)
            ...
            out = [out; [calcReplayStats(activespiketimes,activerates,timebins,trainingfilter(animalnum).output{trainingindex}(epochnum).dist) decodefilter(animalnum).output{1}(epochnum).eventimmobiletime(eventindex) length(activespiketimes) decodefilter(animalnum).output{1}(epochnum).eventdata(eventindex).tdratio decodefilter(animalnum).output{1}(epochnum).eventdata(eventindex).pos]];  %decodefilter(animalnum).output{1}(epochnum).std(eventindex)

            %out = [out; [calcReplayStats(activespiketimes,activerates,timebins,trainingfilter(animalnum).output{trainingindex}(epochnum).dist) decodefilter(animalnum).output{1}(epochnum).eventimmobiletime(eventindex) length(activespiketimes) decodefilter(animalnum).output{1}(epochnum).eventdata(eventindex).meanisodist decodefilter(animalnum).output{1}(epochnum).eventdata(eventindex).meanlratio decodefilter(animalnum).output{1}(epochnum).eventdata(eventindex).stability]];  %decodefilter(animalnum).output{1}(epochnum).std(eventindex)
